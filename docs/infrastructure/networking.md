...
**Wait strategy**: Helm `wait` is disabled and Helm hooks are skipped (`disable_hook: true`) — the chart's only hook (`hubble-generate-certs` post-install Job) cannot schedule during first boot because only the tainted control plane node exists. Two targeted Ansible tasks then poll for the Cilium DaemonSet and operator Deployment to become Ready (up to 10 minutes). Hubble Relay is deliberately not waited on because it depends on TLS certs generated by a CronJob that may not have run yet — it comes up automatically once the cert secret exists. After the targeted waits, the role manually triggers the `hubble-generate-certs` CronJob so Hubble Relay gets its TLS certs without waiting for the next schedule.

**Bootstrap timing**: During first boot, CRI-O tries Dragonfly P2P mirrors (which proxy through Harbor) before the upstream registries. Since CoreDNS needs Cilium and Cilium needs container images, Dragonfly and Harbor mirrors are initially unreachable. Two OS-level tunings prevent this from stalling:
- `net.ipv4.tcp_syn_retries=3` — TCP connections to unreachable mirrors fail in ~15 seconds instead of the default ~130 seconds
- CRI-O `pull_progress_timeout="2m"` — cancels any pull that makes zero progress for 2 minutes

These settings are configured by the `setup_os` and `distribute_pki` roles respectively, and ensure CRI-O falls back to upstream registries within seconds during initial cluster bootstrap.
...
Since the CA is self-signed, browsers will show an untrusted certificate warning. At the end of a full `setup-clusters.py` run, the final play prints the root CA certificate in PEM format along with copy-paste ready import scripts for Windows (PowerShell), Linux, and macOS. Each script embeds the certificate inline, writes it to a temp file, imports it into the OS trust store, and cleans up — no manual file saving needed.

To re-display these instructions later, run `python3 expose-ca.py`.

Alternatively, you can extract the CA certificate manually:
