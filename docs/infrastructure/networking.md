...
**Bootstrap timing**: During first boot, CRI-O tries Harbor proxy-cache mirrors before the upstream registries. Since CoreDNS needs Cilium and Cilium needs container images, mirrors are initially unreachable. Two OS-level tunings prevent this from stalling:
