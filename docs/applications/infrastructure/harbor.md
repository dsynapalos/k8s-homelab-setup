...
- **Delivery**: ArgoCD Application (infra tier, sync-wave 2) via Helm chart `harbor` from `helm.goharbor.io`
- **Namespace**: `harbor`
...
# Quay.io
harbor.k8s.local/quay-cache/thanos/thanos:v0.41.0
...
| CVE reporter | `0 9 * * 1` (weekly Monday 09:00) | Posts Critical/High/Medium CVE CSV to Matrix `#alerts` (see [Security](../../infrastructure/security.md#cve-reporting)) |
...
- **Admin account**: Local `admin` user exists (Harbor architectural requirement) but the password is auto-generated by Ansible (`bootstrap_harbor_secret` role), stored in a K8s Secret (`harbor-admin-password`), and never exposed to humans. Used only by the bootstrap Job, artifact indexer CronJob, and CVE reporter CronJob for headless API calls.

### Dragonfly P2P Preheat (Conditional)

When Dragonfly is deployed (`ENABLE_DRAGONFLY=true`), the bootstrap Job automatically configures Harbor to preheat images via Dragonfly's P2P network. The Job detects Dragonfly by DNS-resolving `dragonfly-manager.dragonfly-system.svc.cluster.local` — if the service doesn't exist, preheat configuration is skipped entirely.

**What preheat does**: When a new image is pushed or pulled through a proxy cache project, Harbor notifies the Dragonfly manager via webhook. The manager instructs seed clients to download and cache the image layers in advance, so subsequent pulls from any node are served from the local P2P network instead of going to the upstream registry.

**What the bootstrap Job configures**:

1. **Preheat instance**: Registers Dragonfly as a preheat provider at `http://dragonfly-manager.dragonfly-system.svc.cluster.local:65003` (vendor: `dragonfly`, enabled, default)
2. **Preheat policies**: Creates an event-based `dragonfly-preheat` policy on each proxy cache project (`dockerhub-cache`, `quay-cache`, `k8s-registry-cache`, `nvcr-cache`) — triggers preheat on every image push/pull event

Both the instance registration and policy creation are idempotent (check-before-create).

**Dragonfly manager TLS trust**: The Dragonfly manager is configured (in its Helm values) with the homelab root CA certificate so it can pull from Harbor over HTTPS. See [Dragonfly — TLS Trust](dragonfly.md#tls-trust) for the full trust chain.
...
- **Dragonfly** (conditional): P2P preheat — bootstrap Job registers Dragonfly as a preheat provider and creates event-based preheat policies on all proxy cache projects (only when Dragonfly is deployed)
- **Matrix (CVE reporter)**: `harbor-cve-reporter` CronJob reads the `matrix-bot` Secret from `monitoring` namespace (via RBAC) and posts a weekly CSV of all Critical/High/Medium CVEs to the `#alerts` room as a file attachment (see [Security — CVE Reporting](../../infrastructure/security.md#cve-reporting))
...
cosign verify --certificate-identity-regexp='.*' \
  --certificate-oidc-issuer-regexp='.*' docker.io/otel/opentelemetry-collector-contrib:0.145.0
...
# Manual test run
kubectl create job --from=cronjob/harbor-cve-reporter cve-reporter-manual -n harbor
kubectl logs -f job/cve-reporter-manual -n harbor

# Common issues:
# - RBAC error reading matrix-bot Secret → Check Role/RoleBinding in monitoring namespace
# - Harbor API timeout → Scans may still be running; wait for daily scan-all to complete
# - Matrix message not appearing → Verify matrix-bot Secret exists in monitoring namespace
```
