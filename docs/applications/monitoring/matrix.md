...
The ArgoCD Application lives in the platform tier of the app-of-app-of-apps hierarchy (`argocd_applications/cluster-apps/platform/matrix.yaml`), which deploys only after all infra-tier apps (including CNPG operator and cert-manager) are Healthy. Uses `ServerSideApply=true` and retry settings to handle CRD availability timing.
