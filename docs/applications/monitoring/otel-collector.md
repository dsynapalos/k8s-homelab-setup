...
| `otel-metrics-pipeline.yaml` | `otel-metrics-pipeline` | Full metrics pipeline: `receivers.prometheus`, `processors.memory_limiter`, `exporters.prometheus` + `exporters.prometheusremotewrite`, `service.pipelines.metrics` |
...
| Processor | Purpose |
|-----------|---------|
| `memory_limiter` | Caps memory at 512 MiB, checked every 5 seconds, prevents OOM |

### Pipeline

```
prometheus receiver → memory_limiter → prometheusremotewrite exporter
...
**Pod OOMKilled**: The `memory_limiter` processor is set to 512 MiB. If scraping many targets, increase the Deployment resource limits and the `memory_limiter.limit_mib` value in the ConfigMap.
